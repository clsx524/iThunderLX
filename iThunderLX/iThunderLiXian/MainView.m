    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT,0), ^{
        ...
        if ([requestResult length] == 32) {
            ...
            dispatch_async( dispatch_get_main_queue(), ^{
                [login_progress stopAnimation:self];
                [toobaritem_login setLabel:@"登录"];
                [[NSAlert alertWithMessageText:@"登录失败" defaultButton:@"确定" alternateButton:nil otherButton:nil informativeTextWithFormat:@"登录失败，请检查您的用户名和密码！并确保当前的iThunderLX为最新版本。如果仍然无法正常登录，请到官方网站发送反馈。"] runModal];
                [login_ok_button setEnabled:YES];
            });
        }
    });
...
-(IBAction)toolbar_refresh:(id)sender {
    ...
    if (!self.hash || self.hash.length != 32) {
        [[NSAlert alertWithMessageText:@"无法刷新任务列表" defaultButton:@"确定" alternateButton:nil otherButton:nil informativeTextWithFormat:@"请先登录您的迅雷VIP账户！"] runModal];
        return;
    }
...
//   标题栏 - 云端删除
//----------------------------------------
-(IBAction)toolbar_yundelete:(id)sender {
